...
typedef struct {
  ...
  FaithSynth4* pInstance;
} Synth4Gubbins;

#define CLAMP(X, LOW, HIGH) (((X) > (HIGH)) ? (HIGH) : (((X) < (LOW)) ? (LOW) : (X)))


Synth4Gubbins g_synthGubbins = {"lib\\rt_synth_4.dll", 0};

// my guess is, global volume systex.
static void doInitJuju4(void* pSynth) {
	FaithSynth4* pInstance = (void*)pSynth; 
  ...
  static BYTE magicJuju2[8] = {0xF0, 0x7F, 0x7F, 0x04, 0x02, 0x00, 0x40, 0xF7};
	pInstance->p1->fpKindaUseless1(pInstance->p1, 64);
	pInstance->p1->fpSysexFeed(pInstance->p1, 0, magicJuju1, 8);
	pInstance->p1->fpSysexFeed(pInstance->p1, 0, magicJuju2, 8);
}




void cbAudioRenderSynth4(void* buffer, int numSamples) {
  int32_t tBuf[SYNTH2_RENDERBUF*2] = {0};
  FaithSynth4* pInstance = g_synthGubbins.pInstance;
  
  assert(numSamples == SYNTH2_RENDERBUF);
  pInstance->p1->fpRender(pInstance->p1, tBuf);

  for (int i=0; i<SYNTH2_RENDERBUF*2; i++) ((int16_t*)buffer)[i] = CLAMP(tBuf[i], -32768, 32767);
}

BOOL synth_init(Synth4Gubbins* pSc) {
  DWORD dummy = 0;
...
void CALLBACK cbMidiSynth2(HMIDIIN hMidiIn, UINT wMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2) { 
  ...
  switch (wMsg) {
      ...
      case MIM_DATA:
        winmmout_enterCrit();
        pInstance->p1->fpMidiFeed(pInstance->p1, 0, (BYTE*)&dwParam1, 1);
        winmmout_leaveCrit();
        break;
      case MIM_LONGDATA:
      case MIM_LONGERROR:
        ...
        if (wMsg == MIM_LONGDATA) {
          ...
          pInstance->p1->fpSysexFeed(pInstance->p1, 0, &m->sysexBuf, 8);
          winmmout_leaveCrit();
        }
...
int main(int argc, char* argv[]) {
  ...
  if (!strcmp(argv[1], "-h")) {
    ...
    if (!winmmout_openMixer(&wf, &cbAudioRenderSynth4)) {
